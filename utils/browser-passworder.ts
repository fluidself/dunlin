 ...
 */
export function encryptWithKey<R>(key: CryptoKey, dataObj: R): Promise<EncryptionResult> {
 ...
 * @param {any} text - cypher text to decrypt
 */
export function decrypt<R>(password: string, text: any): Promise<R> {
  const payload = typeof text === 'object' ? text : JSON.parse(text);
  const { salt } = payload;
 ...
 */
export function decryptWithKey<R>(key: CryptoKey, payload: EncryptionResult): Promise<R> {
